...
class NotificationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.post = Post.objects.create(owner=self.user1, content='Test Post')
    ...
    def test_create_like_notification(self):
        ...
        notification = Notification.objects.get(recipient=self.user1, sender=self.user2, notification_type='like')
    ...
    def test_create_comment_notification(self):
        ...
        # Check that the notification was created
        notification = Notification.objects.get(recipient=self.user1, sender=self.user2, notification_type='comment')
        self.assertIsNotNone(notification)
        self.assertEqual(notification.post, self.post)
        self.assertEqual(notification.recipient, self.user1)
        self.assertEqual(notification.sender, self.user2)
    ...
    def test_single_notification_on_like(self):
        ...
        notifications = Notification.objects.filter(recipient=self.user1, sender=self.user2, notification_type='like')
        self.assertEqual(notifications.count(), 1)

    def test_create_multiple_comment_notifications(self):
        # Create two Comment instances to trigger the signal twice
        comment1 = Comment.objects.create(owner=self.user2, post=self.post, content='Nice post!')
        comment2 = Comment.objects.create(owner=self.user2, post=self.post, content='Another comment!')

        # Check that two notifications were created
        notifications = Notification.objects.filter(recipient=self.user1, sender=self.user2, notification_type='comment')
