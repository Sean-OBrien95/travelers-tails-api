...
class CommentTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.post1 = Post.objects.create(
            owner=self.user1, content='Post 1 content')
        self.post2 = Post.objects.create(
            owner=self.user2, content='Post 2 content')

    def test_comment_creation(self):
        # Create a comment
        comment = Comment.objects.create(
            owner=self.user1, post=self.post1, content='This is a comment')

        # Check if the comment is created/related to the correct user and post
    ...
    def test_comment_ordering(self):
        comment1 = Comment.objects.create(
            owner=self.user1, post=self.post1, content='First comment')
        comment2 = Comment.objects.create(
             owner=self.user2, post=self.post1, content='Second comment')
    ...
    def test_comment_update(self):
        comment = Comment.objects.create(
             owner=self.user1, post=self.post1, content='Original comment')
